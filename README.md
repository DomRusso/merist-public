# Merist HQ — public product website

Official public Merist product presence for reviewers and the public.

## Application (root)

- [Merist HQ application](index.html) — reviewer-facing product shell
  (Overview, platforms, TikTok, Content Posting, analytics, video history, security)

## Documentation pages

- [Product](product.html)
- [How it works](how-it-works.html)
- [TikTok integration](tiktok.html)
- [Supported integrations](platforms.html)
- [Security and data handling](security.html)
- [Terms of Service](terms.html)
- [Privacy Policy](privacy.html)
- [Support / contact](support.html)

## Alpha AORE (separate product)

- [Alpha AORE public info](alpha/index.html)
- [Alpha AORE Terms](alpha/terms.html)
- [Alpha AORE Privacy](alpha/privacy.html)

Alpha pages must not be described as Merist. They exist so the Alpha X
Developer App has an honest Website / Terms / Privacy URL.

## Public URLs (after push to DomRusso/merist-public)

- `https://meristhq.com/`
- `https://meristhq.com/tiktok.html`
- `https://meristhq.com/privacy.html`
- `https://meristhq.com/terms.html`
- `https://meristhq.com/support.html`

## Honesty

- Privacy Policy, Terms of Service, and Support are visible on every page header and footer without opening a menu.
- Demonstration / example values are labeled. Live OAuth and publish are not executed from GitHub Pages.
- Production Content Posting audit approval is not claimed until TikTok grants it.
- Unrestricted autonomous TikTok publishing remains disabled.
